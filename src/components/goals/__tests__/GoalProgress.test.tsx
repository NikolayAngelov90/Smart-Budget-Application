...
describe('GoalProgress', () => {
  it('renders current and target amounts', () => {
    renderWithChakra(<GoalProgress currentAmount={250} targetAmount={1000} currency="EUR" />);
  ...
  it('shows "Completed!" badge when current_amount equals target_amount', () => {
    renderWithChakra(<GoalProgress currentAmount={1000} targetAmount={1000} currency="EUR" />);
    expect(screen.getByText('Completed!')).toBeInTheDocument();
    expect(screen.queryByText(/% complete/)).not.toBeInTheDocument();
  });

  it('shows "Completed!" badge when current_amount exceeds target_amount', () => {
    renderWithChakra(<GoalProgress currentAmount={1200} targetAmount={1000} currency="EUR" />);
    expect(screen.getByText('Completed!')).toBeInTheDocument();
  });

  it('shows 0% when current amount is zero', () => {
    renderWithChakra(<GoalProgress currentAmount={0} targetAmount={1000} currency="EUR" />);
    expect(screen.getByText('0% complete')).toBeInTheDocument();
  });

  it('formats amounts with empty currency fallback', () => {
    renderWithChakra(<GoalProgress currentAmount={250} targetAmount={1000} currency="" />);
    expect(screen.getByText(/250\.00/)).toBeInTheDocument();
    expect(screen.getByText(/1000\.00/)).toBeInTheDocument();
  });

  it('caps percentage at 100 when over-funded', () => {
    renderWithChakra(<GoalProgress currentAmount={1500} targetAmount={1000} currency="EUR" />);
